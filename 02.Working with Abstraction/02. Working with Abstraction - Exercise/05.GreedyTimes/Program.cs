...
public class Program
{
        ...
        for (int i = 0; i < itemQuantityPairs.Length; i += 2)
        {
            ...
            switch (item)
            {
                case "Gem":
                    ...
                    isFull = CheckQuantity(bag, quantity, item, isFull, nextItem);
                    if (isFull)
                    {
                        continue;
                    }
    ...
    private static bool CheckQuantity(Dictionary<string, Dictionary<string, long>> bag, 
                                      long quantity, string item, bool isFull,
                                      string nextItem)
    {
        if (!bag.ContainsKey(item))
        {
            if (bag.ContainsKey((nextItem)))
            {
                if (quantity > bag[nextItem].Values.Sum())
                {
                    isFull = true;
                }
            }
        ...
        else if (bag[item].Values.Sum() + quantity > bag[nextItem].Values.Sum())
        {
            isFull = true;
        }
    ...
    private static void FullQuantityInItem(Dictionary<string, Dictionary<string, long>> bag, 
                                           string name, long quantity, string item)
    {
